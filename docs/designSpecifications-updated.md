...
- **Modals (UPDATED):** native `<dialog>` element, event-delegated JS in `assets/js/dialog.js` (no framework). Used on the Exposure Series detail page: clicking a photo opens the real full-color photo (pulled from CloudFront) sized to its own natural dimensions — **not** a fixed aspect-ratio box, since source photos are all sorts of shapes/dimensions (confirmed originals up to 1140×1152); it's only capped so it can't overflow the viewport. Whichever capture specs (camera/lens/exposure/aperture/ISO/captured) that specific photo actually has are laid out in a wrapping row at the bottom — a photo missing some fields (a fixed-lens camera has no "lens" value; some photos have no EXIF at all) just shows fewer of them or none, not blank/broken-looking placeholders. Dismissing: click the × button, click the photo itself, click the darkened backdrop, or press Escape — all four are equivalent. The dialog deliberately has no explicit CSS `position` override, relying on the browser's native centered/fixed `dialog:modal` behavior, and explicitly restores keyboard focus to whatever opened it on close — both were needed to stop the page scrolling to the top when a dialog closed.
